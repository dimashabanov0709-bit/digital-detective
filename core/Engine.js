...
class Engine {
    ...
    _handleDialogue(step) {
        ...
        // ── Show ONLY the speaking character, hide everyone else ──────────
        // (narrator has no sprite — clear all slots)
        if (step.character === 'narrator') {
            this._clearAllSlots();
        } else {
            this._hideAllExcept(step.character);
        }
    ...
    /* Hide every sprite slot EXCEPT the given characterId.
       The speaking character keeps its current position + animation intact. */
    _hideAllExcept(keepCharacterId) {
        for (const pos of ['left', 'center', 'right']) {
            if (this._slotOccupant[pos] && this._slotOccupant[pos] !== keepCharacterId) {
                const slot = this._getSlot(pos);
                if (slot) {
                    // Fade out smoothly instead of instant removal
    ...
    _startViewportScaler() {
        const W = 1280, H = 720;
        const c = this.dom.gameContainer;

        const doScale = () => {
            // clientWidth/clientHeight — most reliable cross-browser incl. mobile Safari
            ...
            c.style.left            = Math.max(0, (vw - W * s) / 2) + 'px';
            c.style.top             = Math.max(0, (vh - H * s) / 2) + 'px';
        ...
        window.addEventListener('orientationchange', () => {
            doScale(); setTimeout(doScale, 300); setTimeout(doScale, 700);
        });
    }
