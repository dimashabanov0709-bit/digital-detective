...
class Engine {
    ...
    _startViewportScaler() {
        ...
        const scale = () => {
            // clientWidth/clientHeight are the most reliable on all browsers + mobile
            const vw = document.documentElement.clientWidth;
            const vh = document.documentElement.clientHeight;

            // Scale to fit inside the screen keeping 16:9 ratio
            const s = Math.min(vw / W, vh / H);

            // Center the game on screen
            const left = Math.max(0, (vw - W * s) / 2);
            const top  = Math.max(0, (vh - H * s) / 2);
        ...
        window.addEventListener('orientationchange', () => { scale(); setTimeout(scale, 300); setTimeout(scale, 800); });
        if (window.screen?.orientation) {
            window.screen.orientation.addEventListener('change', () => setTimeout(scale, 400));
        }
    }
